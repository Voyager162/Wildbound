...
- F3 toggles a crisp, screen-space debug panel with world/tile coordinates, climate values, world time, current landmark, the nearest landmark type plus edge distance and compass direction, movement mode, seed, chunk position, inventory usage, and FPS.
...
- Six rare, seed-deterministic landmark locations — ancient trees, waterfalls, meteor craters, stone circles, giant skeletons, and watchtowers — are built from large terrain-integrated structures, ground treatments, biome-aware color and detail, and locally varied silhouettes instead of map-like icons. Their exact blocks, roots, rock faces, bones, paths, scars, deposits, scale, rotation, and arrangement regenerate from the world seed. Stone circles use individually varied protruding blocks while keeping their open center's biome terrain, layered grass, ordinary features, and seeded rare deposits intact.
- Ancient trees, waterfalls, and watchtowers have entrances. Approach one to see `Press E to enter`; it opens a separate deterministic hollow-tree, hidden-grotto, or tower interior generated from the world seed and landmark ID. Each interior has its own navigable rooms, passages, exit, lighting, decoration, and rare-material layout.
- Landmark materials are location-specific: ancient trees contain ancient wood, amber sap, glow spores, vine fiber, and heartwood; waterfall grottos contain damp crystal, moss fiber, spring stone, and luminous mushrooms; watchtowers contain map fragments, mechanical parts, and lens/glass materials. Meteor craters expose starstone, meteor iron, and glowing fragments; stone circles hold rune stone, ancient fragments, and relic materials; giant skeletons expose bone fragments, fossil resin, and ancient remains.
...
- Press `E` to open or close the inventory. It has six quick-access slots and a 5 × 5 storage grid. Contextual actions take priority: `E` picks up a nearby dropped item and enters or exits a prompted landmark interior.
...
- The game automatically persists the seed, player position, inventory, equipped tool, harvested features, harvested landmark materials, active cave or landmark-interior return point, and dropped world items. Procedural terrain and landmark layouts themselves are regenerated from the saved seed.
...
- `src/world/landmarkConfig.ts` controls landmark rarity, placement spacing, footprints, and visibility range. `src/world/landmarks/landmarkSurfaceGenerator.ts` and `src/world/landmarks/landmarkInteriorGenerator.ts` own the seeded structure, entrance, material, and interior layouts.
...
Run the deterministic landmark contract and performance verification:

```powershell
& "C:\Program Files\nodejs\npm.cmd" run verify:landmarks
```

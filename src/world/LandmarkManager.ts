...
const drawStoneCircle = (seed: string, visual: LandmarkVisual, palette: LandmarkPalette): void => {
  ...
  blocks.forEach((part, blockIndex) => {
    ...
    const groundContactY = bottomY + Math.max(8, screenDepth * 0.22);
    ...
    structure.fillEllipse(center.x - screenWidth * 0.47, groundContactY + 0.5, screenWidth * 0.45, 4);
    structure.fillEllipse(center.x + screenWidth * 0.47, groundContactY + 0.5, screenWidth * 0.45, 4);
    for (let blade = 0; blade < 44; blade += 1) {
      ...
      const bladeBaseY = groundContactY
        + detailRandom(seed, landmark, blockIndex * 47 + blade, 0xc454) * 0.75;
      const bladeHeight = 8 + detailRandom(seed, landmark, blockIndex * 41 + blade, 0xc451) * 15;
      structure.fillStyle(rootColor, 0.98);
      structure.fillEllipse(bladeBaseX, bladeBaseY + 0.4, 5 + (blade % 3), 2.25);
      ...
      if (blade % 5 === 0) {
        ...
        structure.fillEllipse(bladeBaseX - side * 2.5, bladeBaseY - 2.4, 4.5, 2.2);
      }
    }
  });
